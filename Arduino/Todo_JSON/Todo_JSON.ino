...
void loop() {
    ...
    if (EnviarMediaAne3s == true){ // Si se activa la lectura de la media del anemómetro de 3 segundos...
      ...
      char dia[8];
      sprintf(dia, "%04d%02d%02d", dt.year, dt.month, dt.day);
      JsonObject& s1 = jsonBuffer.createObject();// Dia del dato
      s1["Dia"] = String(dia);
      char hora[6];
      sprintf(hora, "%02d%02d%02d", dt.hour, dt.minute, dt.second);
      JsonObject& s2 = jsonBuffer.createObject(); // Hora del dato
      s2["Hora"] = String(hora);
      JsonObject& st = jsonBuffer.createObject(); // Tipo dato
      st["Tipo"] = "2"; //Json con 2 datos
      JsonArray& datosS1 = json.createNestedArray("Fecha");
      datosS1.add(s1);
      datosS1.add(s2);
      datosS1.add(st);
